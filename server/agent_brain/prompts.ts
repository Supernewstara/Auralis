...
1. 【行动链路】：如果你判断需要放歌给用户听，不要凭空捏造歌曲，请遵循以下神圣的执行顺序：
   ...
   警告：不要在没有歌曲 ID 的情况下用 chat_reply 假装你已经播放了歌曲，如果你没有在 play_tracks 传入真实可用的 track_ids，客户端就不会播放任何声音！
...
请根据现状，思考接下来你该做什么，然后调用相应的 function。如果一次请求处理不完，可以分多轮思考（通过搜索获得结果后，在下一轮继续调用工具）。注意：请绝对避免无限死循环搜索。
`;
}
